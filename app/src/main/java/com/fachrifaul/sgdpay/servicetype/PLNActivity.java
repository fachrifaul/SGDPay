...
public class PLNActivity extends AppCompatActivity {
    ...
    public static Intent getStartIntent(Context context, String titleScreen) {
        Intent intent = new Intent(context, PLNActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tabLayout.addTab(tabLayout.newTab().setText("Token"));
    ...
    private void productChoice() {
        new MaterialDialog.Builder(this)
                .title("Pilih Produk")
                .items(R.array.pln_token_values)
                .itemsCallbackSingleChoice(0, new MaterialDialog.ListCallbackSingleChoice() {
                    @Override
                    public boolean onSelection(MaterialDialog dialog, View view, int which, CharSequence text) {
                        String message = "Anda akan membeli PLN no " +  customerNumberEditText.getText().toString() +
                                "DZXXXX (JAN2018) sebesar "+text+" Jika Benar masukan PIN anda.";
                        dialogPin(message);
                        return true;
                    }
                })
    ...
    private void productInquiry() {
        String message = "Anda akan membayar PLN no " +  customerNumberEditText.getText().toString() +
                " DZXXXX (JAN2018) sebesar Rp. 123.122. Jika Benar masukan PIN anda.";
        dialogPin(message);
    }
